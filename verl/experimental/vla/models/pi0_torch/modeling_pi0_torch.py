...
class PI0ForActionPrediction(PreTrainedModel, SupportSACTraining, SupportSFTTraining):
    ...
    def bc_loss(
        ...
        action_tensor_raw = action_tensor.clone()
        ...
        noisy_model_input, timesteps = self.loss_func.add_noise(action_tensor)
        model_pred = self.model(images, img_masks, lang_tokens, lang_masks, states, noisy_model_input, timesteps)

        action_loss_mask = (torch.arange(self.model.n_action_steps, device=model_pred.device) < 10).to(model_pred.dtype).unsqueeze(0).expand(model_pred.shape[0], -1)
