...
class InterventionActionProcessorStep(ProcessorStep):
    ...
    """
    # ==== 新增一个内部工具函数，用来按行读取文件 ====
    def _get_next_left_ee_delta_from_file(self):
        """
        从 left_ee_delta_poses.txt 依次读取一行 (6 个值)：
        [left_delta_x, left_delta_y, left_delta_z, left_delta_rx, left_delta_ry, left_delta_rz]

        - 每次调用返回下一行
        - 如果行数用完，则一直复用最后一行（并打印一个 warning）
        """
        import os

        # 文件路径（按你给的路径）
        file_path = "/home/agilex-home/agilex/lerobot_hil-serl/src/lerobot/motors/piper/left_ee_delta_poses.txt"

        # 懒加载：第一次调用时读文件，存成 numpy 数组
        if not hasattr(self, "_left_ee_delta_poses"):
            if not os.path.exists(file_path):
                raise FileNotFoundError(f"left_ee_delta_poses file not found: {file_path}")
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if len(parts) < 6:
                        raise ValueError(f"Line in {file_path} does not have 6 values: {line}")
            ...
            self._left_ee_delta_poses = np.asarray(poses, dtype=float)
            self._left_ee_delta_idx = 0

            #print(f"[InterventionActionProcessorStep] Loaded {self._left_ee_delta_poses.shape[0]} lines "
            #      f"from {file_path}")

        # 取当前索引
        idx = getattr(self, "_left_ee_delta_idx", 0)
        n = self._left_ee_delta_poses.shape[0]

        if idx >= n:
            # 用完了就一直用最后一行，避免崩掉
            idx = n - 1
            # print(
            #     "[InterventionActionProcessorStep] WARNING: "
            #     "left_ee_delta_poses.txt lines exhausted, reusing last line."
            # )
        else:
            # 正常前进一行
            self._left_ee_delta_idx = idx + 1

        return self._left_ee_delta_poses[idx]  # shape: (6,)
    ...
    def __call__(self, transition: EnvTransition) -> EnvTransition:
        ...
        # Debug: 打印一次干预状态和 teleop_action 概要，帮助排查是否进入了干预分支
        # print(
        #     "[InterventionActionProcessorStep] "
        #     f"is_intervention={is_intervention}, "
        #     f"terminate_episode={terminate_episode}, success={success}, "
        #     f"has_teleop_action={teleop_action is not None and teleop_action != {}}"
        # )
        ...
        if is_intervention and teleop_action is not None:
            if isinstance(teleop_action, dict):
                # # ========= 从文件中按行读取左臂末端增量位姿 =========
                # lx, ly, lz, lroll, lpitch, lyaw = self._get_next_left_ee_delta_from_file()

                # # 1) 平移：从 mm -> m（假设原始单位是 mm）
                ...
                # # 3) 组装 action_list：
                # #    顺序与原来代码注释保持一致：
                # #    [left_xyz, left_rxyz, left_gripper, right_xyz, right_rxyz, right_gripper]
                # action_list = [
                #     # 左臂平移（单位：m）
                #     lx_m, ly_m, lz_m,
                #     # 左臂姿态（单位：rad）
                ...
                #帮我把这里改成读取/home/agilex-home/agilex/lerobot_hil-serl/src/lerobot/motors/piper/left_ee_delta_poses.txt的每一行。
                #要求1、每次调用这个函数，都换行，比如第一次调用这个函数读取上面文本文件的第一行，第二次调用就读取第二行，第三次就第三行等等
                #2、上述文本文件每一行只有6个值，对应left_delta_x，left_delta_y，left_delta_z，left_delta_rx，left_delta_ry，left_delta_rz。其他没有的值直接置0就行
                #3、上述文本前三个位移值需要像这样lx_m, ly_m, lz_m = lx / 1000.0, ly / 1000.0, lz / 1000.0，转换为m做单位。后三个值要想lrx, lry, lrz = np.deg2rad([lroll, lpitch, lyaw])这样转换为弧度做单位
                action_list = [
                    # 左臂平移
                    teleop_action.get("left_delta_x", 0.0),
                    teleop_action.get("left_delta_y", 0.0),
                    teleop_action.get("left_delta_z", 0.0),
                    # 左臂姿态
                    teleop_action.get("left_delta_rx", 0.0),
                    teleop_action.get("left_delta_ry", 0.0),
                    teleop_action.get("left_delta_rz", 0.0),
                    # 左臂夹爪
                    teleop_action.get("left_delta_gripper", 0.0),

                    # 右臂平移
                    teleop_action.get("right_delta_x", 0.0),
                    teleop_action.get("right_delta_y", 0.0),
                    teleop_action.get("right_delta_z", 0.0),
                    # 右臂姿态
                    teleop_action.get("right_delta_rx", 0.0),
                    teleop_action.get("right_delta_ry", 0.0),
                    teleop_action.get("right_delta_rz", 0.0),
                    # 右臂夹爪
                    teleop_action.get("right_delta_gripper", 0.0),
                ]
                #print("[InterventionActionProcessorStep] action_list from teleop:", action_list)
            ...
            # print(
            #     "[InterventionActionProcessorStep] overriding ACTION with tensor:",
            #     teleop_action_tensor.detach().cpu().numpy()
            # )
