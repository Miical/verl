...
class VanillaObservationProcessorStep(ObservationProcessorStep):
    ...
    **State Processing:**
    ...
    def _process_observation(self, observation):
        ...
        # pdb.set_trace()
        if "agent_end_pose_value" in processed_obs:
            agent_end_pose_value_np = processed_obs.pop("agent_end_pose_value")
            agent_end_pose_value = torch.from_numpy(agent_end_pose_value_np).float()
            if agent_end_pose_value.dim() == 1:
                agent_end_pose_value = agent_end_pose_value.unsqueeze(0)
            processed_obs[OBS_STATE] = agent_end_pose_value
