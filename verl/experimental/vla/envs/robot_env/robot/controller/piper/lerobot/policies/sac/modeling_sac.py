...
class SACPolicy(
    ...
    def compute_loss_actor(
        ...
        # # 保证不反传到数据张量
        # a_demo = action_bc.detach()
        # # pdb.set_trace()
        # # 若你的动作维度包含离散维（例如抓手开闭），这里只对连续维做 BC
        # if self.config.num_discrete_actions is not None:
        #     a_demo = a_demo[:, :DISCRETE_DIMENSION_INDEX]
        #     bc_l2 = F.mse_loss(actions_pi, a_demo)
