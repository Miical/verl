...
# ============================================================================
MINI_BATCH_SIZE=128                            # mini batch 大小（每个 GPU 的批次大小）
                                               # 在 SAC 中目前无效
...
$PYTHON -m verl.experimental.vla.main_sac \
    ...
    `# 从脚本 A 引入：gym_testenv 环境配置（使用 LeRobot 数据集回放）` \
    ...
    `# 从脚本 A 引入：真机环境不需要机器人和遥操作配置（设为 null）` \
