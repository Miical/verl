...
class RealRobotEnvWrapper:
    ...
    def _init_kinematics(self):
        ...
        try:
            ...
            # 使用相对路径：相对于当前文件 (piper_env.py) 的位置
            # piper_env.py 位于 .../robot/controller/piper/piper_env.py
            # local_assets 位于 .../robot/controller/piper/local_assets/
            current_file = Path(__file__).resolve()
            piper_dir = current_file.parent  # .../robot/controller/piper/
            
            # 🔧 修复：使用 local_assets/robot.urdf（直接文件），而不是嵌套路径
            # 这样 kinematics.py 会切换到 local_assets/ 目录，就能找到 meshes/ 子目录了
            urdf_path = piper_dir / "local_assets" / "robot.urdf"
            ...
            force_print(f"[RealRobotEnvWrapper] URDF file exists: {urdf_path.exists()}")
            
            # 检查 URDF 文件是否存在
            if not urdf_path.exists():
                force_print(f"[RealRobotEnvWrapper] WARNING: URDF file not found at {urdf_path}")
                # 尝试使用嵌套路径作为后备
                nested_urdf = piper_dir / "local_assets" / "piper.urdf" / "robot.urdf" / "robot.urdf"
                if nested_urdf.exists():
                    force_print(f"[RealRobotEnvWrapper] Using nested URDF path: {nested_urdf}")
                    urdf_path = nested_urdf
            
            # kinematics.py 会自动切换到 URDF 文件所在目录，所以不需要手动切换
    ...
    def reset(self, episode_id: int | None = None):
        ...
        # 注意：只有 TestRobotEnv（gym_testenv 模式）才有 hdf5_loader
        # RobotEnv（真实机器人模式）没有 hdf5_loader，直接跳过
