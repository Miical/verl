...
class RobotKinematics:
    ...
    def __init__(
        ...
        # placo.RobotWrapper 需要传入包含 robot.urdf 的目录路径
        # 它会自动在目录中查找 robot.urdf 文件
        import os
        from pathlib import Path
        
        # 🔧 关键修复：使用绝对路径并切换工作目录
        # placo 在加载 URDF 时会基于当前工作目录查找 mesh 文件
        urdf_path_obj = Path(urdf_path).resolve()
        
        if urdf_path_obj.is_file() and urdf_path_obj.suffix == '.urdf':
            # 传入的是 URDF 文件路径，取其父目录
            urdf_dir = urdf_path_obj.parent
        elif urdf_path_obj.is_dir():
            # 传入的是目录路径
            urdf_dir = urdf_path_obj
        else:
        ...
        try:
            # 🔧 切换到 URDF 所在目录（这样 placo 可以找到 meshes/ 子目录）
            os.chdir(str(urdf_dir))
            
            # placo.RobotWrapper 需要传入目录路径（不是文件路径）
            # 它会在该目录下查找 robot.urdf
            self.robot = placo.RobotWrapper(str(urdf_dir))
        finally:
            # 恢复原工作目录
