...
import os
import time
from pyorbbecsdk import FormatConvertFilter, VideoFrame, ColorFrame
...
def frame_to_numpy_image(frame: VideoFrame) -> Union[Optional[np.array], Any]:
    width = frame.get_width()
    height = frame.get_height()
...
def save_color_frame(frame: ColorFrame, index):
    ...
    image = frame_to_numpy_image(frame)
    if image is None:
        print("failed to convert frame to image")
        return
    cv2.imwrite(filename, image)
    print("saved color frame to {}".format(filename))