# recipe/vla/sac/tests/test_toy_sac_with_your_interfaces_zeroall_rewardmax_chunkgamma.py
# -*- coding: utf-8 -*-
"""
Ablation: PREFIX ZERO + UNUSED ACTIONS ZERO + (reward=max) + (gamma discount once per chunk)

- prefix: ZERO (simulate no vision/lang signal)
- action noise: ONLY executed region has noise; unused (50x32 outside 10x7) are ZERO
- reward: keep per-micro-step rewards (B,10); algorithm takes rewards.max(dim=-1)
- discount: only ONCE per chunk (sac.gamma = gamma_micro), not per micro-step
"""
...
# ----------------------------
# Fake PI0Model stub (PREFIX ZERO + UNUSED ACTIONS ZERO)
# ----------------------------
class FakePI0Model(nn.Module):
    """
    Minimal stub satisfying PI0ForActionPrediction SAC path:
      - embed_prefix(...) -> prefix_embs are ZERO
      - sample_noise(...) -> ONLY executed slice has randn; unused dims are ZERO
      - denoise_step(...) -> predicts v only for executed slice; rest v=0
    """
    ...
    def sample_noise(self, actions_shape, device):
        # ✅ unused actions are ZERO; only executed slice has noise
...
@torch.no_grad()
def collect_batch_rewardmax(policy, env, B, exec_steps, exec_dim, device):
    """
    This version:
      - stores rewards_seq (B,10) directly
      - lets your algorithm do rewards.max(dim=-1) internally
    ...
    batch = TensorDict(
        {
            ...
            "rewards": rewards_seq.to(torch.float32),  # (B,10) -> algo will take max
...
def rollout_and_render(policy, exec_steps, exec_dim, out_dir,
    ...
    for _chunk in range(episode_chunks):
    ...
    for cs in chunk_start_indices:
        ...
        # remaining micro-steps in default
        if cs + 1 < ce:
            x = pos[cs+1:ce+1, 0]
            y = pos[cs+1:ce+1, 1]
...
def main():
    ...
    parser.add_argument("--updates", type=int, default=200)          # ✅ 训练轮数
    ...
    parser.add_argument("--gamma_micro", type=float, default=0.5)  # ✅ chunk-level gamma
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--smooth", type=int, default=10)
    parser.add_argument("--out_dir", type=str, default="toy_plots_zeroall_rewardmax_chunkgamma")
    ...
    cfg = OmegaConf.create(
        {
            ...
            "replay_pool_save_dir": f"/tmp/toy_replay_pool_zeroall_rewardmax_chunkgamma_seed{args.seed}",
    ...
    for step in range(1, args.updates + 1):
        batch, dbg = collect_batch_rewardmax(
            policy=policy,
            env=env,
            B=args.batch_size,
            exec_steps=args.exec_steps,
        ...
        # extra debug: action quality vs target mu=-state
        with torch.no_grad():
            s0 = batch["s0.states"]
            s0_obs = make_dummy_obs(s0, device)
            sf = policy.sac_forward_state_features(s0_obs)
        ...
        metrics["debug/reward_seq_mean"] = dbg["reward_seq_mean"]
