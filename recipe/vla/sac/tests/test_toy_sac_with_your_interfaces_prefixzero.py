# recipe/vla/sac/tests/test_toy_sac_with_your_interfaces_prefixzero.py
# -*- coding: utf-8 -*-
"""
Prefix-zero SAC toy (ablation):
...
- prefix: ZERO (simulate "vision/lang features removed")
- actions: full 50x32 noise (unused steps/dims are NOT zeroed)  <-- IMPORTANT
"""
...
# ----------------------------
# Fake PI0Model stub (PREFIX ZERO, ACTION NOISY)
# ----------------------------
class FakePI0Model(nn.Module):
    """
    Same API, but:
      - prefix_embs is ZERO   (only change vs ORIGINAL)
      - sample_noise is full randn (B,50,32)  (keep original noisy actions)
    ...
    def sample_noise(self, actions_shape, device):
        # ✅ keep ORIGINAL: full noise everywhere (unused dims NOT zeroed)
    ...
    batch = TensorDict(
        {
            ...
            "rewards": rewards_seq.to(torch.float32),   # (B,10)
...
def main():
    ...
    parser.add_argument("--out_dir", type=str, default="toy_plots_prefixzero")
    ...
    cfg = OmegaConf.create(
        {
            ...
            "replay_pool_save_dir": f"/tmp/toy_replay_pool_prefixzero_seed{args.seed}",
